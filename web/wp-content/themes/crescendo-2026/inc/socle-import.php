...
function crescendo_socle_get_template(array $data) {
    $template = sanitize_key($data['page']['template'] ?? '');
...
function crescendo_import_socle_from_array(array $data) {
    ...
    $prefix = crescendo_socle_get_field_prefix($data);
    ...
    $pageArgs = array(
        ...
        'page_template' => crescendo_socle_get_template($data),
    );
...
function crescendo_import_socle_from_json_file($file_path) {
    ...
    return crescendo_import_socle_from_array($data);
}

function crescendo_get_socle_import_dir() {
    return crescendo_get_content_import_dir('socle/');
}

function crescendo_list_socle_import_files() {
    $dir = crescendo_get_socle_import_dir();
    if (!is_dir($dir)) {
        return array();
    }
...
function crescendo_register_socle_import_page() {
    add_management_page(
        'Import pages Socle',
        'Import Socle JSON',
        'manage_options',
        'crescendo-socle-import',
        'crescendo_render_socle_import_page'
    );
}
add_action('admin_menu', 'crescendo_register_socle_import_page');

function crescendo_render_socle_import_page() {
    ...
    if (!empty($_POST['crescendo_import_nonce']) && wp_verify_nonce($_POST['crescendo_import_nonce'], 'crescendo_import_socle')) {
        if (!empty($_POST['import_file'])) {
            $file = sanitize_text_field(wp_unslash($_POST['import_file']));
            $allowed = crescendo_list_socle_import_files();
        ...
        if (!empty($_FILES['json_upload']['tmp_name']) && is_uploaded_file($_FILES['json_upload']['tmp_name'])) {
            ...
            if ($extension !== 'json') {
                ...
                if (json_last_error() !== JSON_ERROR_NONE) {
                    ...
                    $result = crescendo_import_socle_from_array($data);
                    if (is_wp_error($result)) {
                        $messages[] = array('type' => 'error', 'text' => $result->get_error_message());
                    } else {
                        $action = $result['updated'] ? 'mise à jour' : 'création';
    ...
    $files = crescendo_list_socle_import_files();
    ?>
    <div class="wrap">
        <h1>Import pages Socle (JSON)</h1>
        <p>Importez les pages À propos, Mentions légales et Politique de confidentialité. Dossier : <code><?php echo esc_html(crescendo_get_socle_import_dir()); ?></code></p>
        ...
        <?php if (empty($files)) : ?>
            <p>Aucun fichier JSON trouvé. Ajoutez des fichiers dans <code>content-import/socle/</code>.</p>
        <?php else : ?>
            <table class="widefat striped">
                ...
                <?php foreach ($files as $file) : ?>
                    <tr>
                        ...
                        <td>
                            <form method="post">
                                <?php wp_nonce_field('crescendo_import_socle', 'crescendo_import_nonce'); ?>
...
if (defined('WP_CLI') && WP_CLI) {
    WP_CLI::add_command('crescendo import-socle', function ($args) {
        $file = $args[0] ?? '';
        if (!$file) {
            WP_CLI::error('Usage: wp crescendo import-socle path/to/file.json');
        }
        $result = crescendo_import_socle_from_json_file($file);
        if (is_wp_error($result)) {
