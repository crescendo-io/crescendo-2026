...
function crescendo_import_secteur_from_array(array $data) {
...
function crescendo_import_secteur_from_json_file($file_path) {
    ...
    return crescendo_import_secteur_from_array($data);
}

function crescendo_get_secteur_import_dir() {
    return crescendo_get_content_import_dir('secteurs/');
}

function crescendo_list_secteur_import_files() {
    $dir = crescendo_get_secteur_import_dir();
    if (!is_dir($dir)) {
        return array();
    }
...
function crescendo_register_secteur_import_page() {
    add_management_page(
        'Import pages Secteur',
        'Import Secteur JSON',
        'manage_options',
        'crescendo-secteur-import',
        'crescendo_render_secteur_import_page'
    );
}
add_action('admin_menu', 'crescendo_register_secteur_import_page');

function crescendo_render_secteur_import_page() {
    ...
    if (!empty($_POST['crescendo_secteur_nonce']) && wp_verify_nonce($_POST['crescendo_secteur_nonce'], 'crescendo_import_secteur')) {
        if (!empty($_POST['import_file'])) {
            $file = sanitize_text_field(wp_unslash($_POST['import_file']));
            if (in_array($file, crescendo_list_secteur_import_files(), true)) {
                $result = crescendo_import_secteur_from_json_file($file);
        ...
        if (!empty($_FILES['json_upload']['tmp_name']) && is_uploaded_file($_FILES['json_upload']['tmp_name'])) {
            ...
            if ($extension !== 'json') {
                ...
                if (json_last_error() !== JSON_ERROR_NONE) {
                    ...
                    $result = crescendo_import_secteur_from_array($data);
                    if (is_wp_error($result)) {
                        $messages[] = array('type' => 'error', 'text' => $result->get_error_message());
                    } else {
    ...
    $files = crescendo_list_secteur_import_files();
    ?>
    <div class="wrap">
        ...
        <?php if (empty($files)) : ?>
            <p>Aucun fichier. Ajoutez des JSON dans <code>content-import/secteurs/</code>.</p>
        <?php else : ?>
            <table class="widefat striped">
                ...
                <?php foreach ($files as $file) : ?>
                    <tr>
                        ...
                        <td>
                            <form method="post">
                                <?php wp_nonce_field('crescendo_import_secteur', 'crescendo_secteur_nonce'); ?>
...
if (defined('WP_CLI') && WP_CLI) {
    WP_CLI::add_command('crescendo import-secteur', function ($args) {
        $file = $args[0] ?? '';
        if (!$file) {
            WP_CLI::error('Usage: wp crescendo import-secteur path/to/file.json');
        }
        $result = crescendo_import_secteur_from_json_file($file);
        if (is_wp_error($result)) {
