...
function crescendo_import_realisations_from_array(array $data) {
    ...
    $pageArgs = array(
        ...
        'page_template' => 'template-realisations.php',
    );
    ...
    if (!empty($data['seo']) && is_array($data['seo'])) {
        foreach ($data['seo'] as $key => $value) {
            $field = 'realisations-seo-' . str_replace('_', '-', sanitize_key($key));
    ...
    if (!empty($data['fields']) && is_array($data['fields'])) {
        foreach ($data['fields'] as $field => $value) {
            if (strpos($field, 'realisations-') !== 0) {
                continue;
            }
            update_field($field, $value, $post_id);
        }
...
function crescendo_import_realisations_from_json_file($file_path) {
    ...
    return crescendo_import_realisations_from_array($data);
}

function crescendo_get_realisations_import_dir() {
    return crescendo_get_content_import_dir('realisations/');
}

function crescendo_list_realisations_import_files() {
    $dir = crescendo_get_realisations_import_dir();
    if (!is_dir($dir)) {
        return array();
    }
...
function crescendo_register_realisations_import_page() {
    add_management_page(
        'Import page Réalisations',
        'Import Réalisations JSON',
        'manage_options',
        'crescendo-realisations-import',
        'crescendo_render_realisations_import_page'
    );
}
add_action('admin_menu', 'crescendo_register_realisations_import_page');

function crescendo_render_realisations_import_page() {
    ...
    if (!empty($_POST['crescendo_import_nonce']) && wp_verify_nonce($_POST['crescendo_import_nonce'], 'crescendo_import_realisations')) {
        if (!empty($_POST['import_file'])) {
            $file = sanitize_text_field(wp_unslash($_POST['import_file']));
            $allowed = crescendo_list_realisations_import_files();
            if (in_array($file, $allowed, true)) {
                $result = crescendo_import_realisations_from_json_file($file);
        ...
        if (!empty($_FILES['json_upload']['tmp_name']) && is_uploaded_file($_FILES['json_upload']['tmp_name'])) {
            ...
            if ($extension !== 'json') {
                ...
                if (json_last_error() !== JSON_ERROR_NONE) {
                    ...
                    $result = crescendo_import_realisations_from_array($data);
                    if (is_wp_error($result)) {
                        $messages[] = array('type' => 'error', 'text' => $result->get_error_message());
                    } else {
                        $action = $result['updated'] ? 'mise à jour' : 'création';
    ...
    $files = crescendo_list_realisations_import_files();
    ?>
    <div class="wrap">
        <h1>Import page Réalisations (JSON)</h1>
        <p>Importez la page réalisations depuis un fichier JSON. Dossier : <code><?php echo esc_html(crescendo_get_realisations_import_dir()); ?></code></p>
        ...
        <?php if (empty($files)) : ?>
            <p>Aucun fichier JSON trouvé. Ajoutez des fichiers dans <code>content-import/realisations/</code>.</p>
        <?php else : ?>
            <table class="widefat striped">
                ...
                <?php foreach ($files as $file) : ?>
                    <tr>
                        ...
                        <td>
                            <form method="post">
                                <?php wp_nonce_field('crescendo_import_realisations', 'crescendo_import_nonce'); ?>
...
if (defined('WP_CLI') && WP_CLI) {
    WP_CLI::add_command('crescendo import-realisations', function ($args) {
        $file = $args[0] ?? '';
        if (!$file) {
            WP_CLI::error('Usage: wp crescendo import-realisations path/to/file.json');
        }
        $result = crescendo_import_realisations_from_json_file($file);
        if (is_wp_error($result)) {
